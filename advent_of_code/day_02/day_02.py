...
PART_1_MATCH_PATTERN = re.compile(r"^(\d+)\1$")
PART_2_MATCH_PATTERN = re.compile(r"^(\d+)(\1)+$")
...
def part_1_is_valid_id(id: int) -> bool:
    return not PART_1_MATCH_PATTERN.search(str(id))


def part_2_is_valid_id(id: int) -> bool:
    return not PART_2_MATCH_PATTERN.search(str(id))


def find_all_invalid_ids(input: str, part_1: bool = True) -> list[int]:
    ...
    for start, finish in parsed:
        for id in range(start, finish + 1):
            if part_1:
                if not part_1_is_valid_id(id):
                    invalid_ids.append(id)
            else:
                if not part_2_is_valid_id(id):
                    invalid_ids.append(id)
...
if __name__ == "__main__":
    ...
    print("Part 2:")
    print(sum(find_all_invalid_ids(INPUT, part_1=False)))
